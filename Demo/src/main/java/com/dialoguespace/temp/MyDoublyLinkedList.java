...
public class MyDoublyLinkedList<E> {
	...
	public void addFirst(E value) {
		...
		// 2. 새 노드를 추가 (이 때 첫 번째 노드니까 prev는 null이 되고 next는 head가 가리키는 노드가 된다)
		Node<E> new_node = new Node<>(null, value, first);
		
		// 3. 노드를 추가하였으니 리스트 크기 증가
		size++;
		
		// 4. 첫 번째 기준이 변경되었으니 head를 새 노드를 참조하도록 업데이트
		head = new_node;
		
		if(first == null) {
			// 5. 만일 빈 리스트에서 최초의 요소 추가였을 경우, tail도 첫 번째 노드를 바라보도록 업데이트
			tail = new_node;
		} else {
			// 6. 만일 빈 리스트가 아닐 경우, 추가되기 이전 첫 번째였던 노드에서 prev를 새 노드로 업데이트
			first.prev = new_node;
		}
	}
	...
	public void addLast(E value) {
		...
		// 2. 새 노드를 추가 (이 때 마지막 노드니까 next는 null이 되고 prev는 tail이 가리키는 노드가 된다)
		Node<E> new_node = new Node<>(last, value, null);
		
		// 3. 노드를 추가하였으니 리스트 크기 증가
		size++;
		
		// 4. 마지막 기준이 변경되었으니 tail을 새 노드를 참조하도록 업데이트
		tail = new_node;
		
		if(last == null) {
			// 5. 만일 빈 리스트에서 최초의 요소 추가였을 경우, head도 첫 번째 노드를 바라보도록 업데이트
			head = new_node;
		} else {
			// 6. 만일 빈 리스트가 아닐 경우, 추가되기 이전 마지막이었던 노드에서 next를 새 노드로 바라보도록 업데이트
			last.next = new_node;
		}
	}
	...
	public void add(int index, E value) {
		// 1. 인덱스 범위 체크
		// 인덱스가 0보다 작거나 size보다 클 경우 에러
		// (인덱스가 size보다 크면 마지막 요소 다음 빈 공간의 다음 빈 공간을 가리키는 것이므로)
		...
		// 4. 추가하려는 위치의 노드가 새 노드의 다음 노드가 됨
	...
	public E removeFirst() {
		...
		if(first == null) {
			...
			// 8. 만일 빈 리스트가 아닐 경우, 삭제되기 이전 두 번째였던 first가 첫 번째 노드가 되도록 업데이트
			first.prev = null;
		}
	...
	public E removeLast() {
		...
		// 2. 삭제 될 마지막 요소의 데이터를 백업
		E value = tail.item;
		
		// 3. 마지막 노드의 이전 노드를 임시 저장
		Node<E> last = tail.prev;
		
		// 4. 마지막 노드의 내부 요소를 모드 삭제
		tail.item = null;
		tail.prev = null;
		...
		if(last == null) {
			...
			// 8. 만일 빈 리스트가 아닐 경우, 삭제되기 이전 마지막의 이전 노드였던 last가 마지막 노드
			last.next = null;
		}
	...
	public E remove(int index) {
		// 1. 인덱스가 0보다 작거나 size 보다 크거나 같은 경우 에러(size와 같을 경우 빈 공간을 가리키는 것)
		if(index < 0 || index >= size) {
	...
	public boolean remove(Object value) {
		...
		// 5. 만약 삭제하려는 노드가 head(첫 번째 요소)라면, 기존 removeFirst() 사용
		if(del_node == head) {
			removeFirst();
			return true;
		}
		
		// 6. 만약 삭제하려는 노드가 tail(마지막 요소)라면, 기존 removeLast() 사용
		if(del_node == tail) {
			removeLast();
			return true;
		}
	...
	public void clear() {
		// i.next가 null인 경우 마지막을 의미하는 것이므로 null이 아닐 때까지 순회
		for(Node<E> i = head; i.next != null;) {
			Node<E> next_node = i.next;	// i에 관한 값을 지울 것이므로 미리 백업
	...
	public boolean contains(Object value) {
		return indexOf(value) != -1;	// --1 이 아니라는 것은 요소가 리스트에 존재한다는 것이다.
	}
